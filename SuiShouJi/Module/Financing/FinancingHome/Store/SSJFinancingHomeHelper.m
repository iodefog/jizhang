...
+ (void)queryForFundingSumMoney:(void(^)(double result))success failure:(void (^)(NSError *error))failure{
    [[SSJDatabaseQueue sharedInstance]asyncInDatabase:^(FMDatabase *db) {
        ...
        double fundingSum = 0;
        NSString *currentDate = [[NSDate date]formattedDateWithFormat:@"yyyy-MM-dd"];
        fundingSum = [db doubleForQuery:@"select sum(a.imoney) from bk_user_charge a, bk_user_bill_type b ,bk_fund_info c where a.ibillid = b.cbillid and a.cuserid = b.cuserid and a.cbooksid = b.cbooksid and a.cuserid = ? and a.operatortype <> 2 and (a.cbilldate <= ? or ichargetype = ?) and b.itype = 0 and a.ifunsid = c.cfundid and c.idisplay = 1 and c.operatortype <> 2 and a.ibillid not in ('13','14')",userid,currentDate,@(SSJChargeIdTypeLoan)] - [db doubleForQuery:@"select sum(a.imoney) from bk_user_charge a, bk_user_bill_type b ,bk_fund_info c where a.ibillid = b.cbillid and a.cuserid = b.cuserid and a.cbooksid = b.cbooksid and a.cuserid = ? and a.operatortype <> 2 and (a.cbilldate <= ? or ichargetype = ?) and b.itype = 1 and a.ifunsid = c.cfundid and c.idisplay = 1 and c.operatortype <> 2 and a.ibillid not in ('13','14')",userid,currentDate,@(SSJChargeIdTypeLoan)];
        if (success) {
            SSJDispatch_main_async_safe(^{
                success(fundingSum);
            });
