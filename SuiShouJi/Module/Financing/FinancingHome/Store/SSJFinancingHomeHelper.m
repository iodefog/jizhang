...
+ (void)queryForFundingListWithSuccess:(void(^)(NSArray<SSJFinancingHomeitem *> *result))success failure:(void (^)(NSError *error))failure {
    [[SSJDatabaseQueue sharedInstance]asyncInDatabase:^(FMDatabase *db) {
        ...
        for (SSJFinancingHomeitem *item in fundingList) {
            item.fundingAmount = [db doubleForQuery:@"select sum(a.imoney) from bk_user_charge a, bk_bill_type b left join bk_share_books_member c on c.cbooksid = a.cbooksid and c.cmemberid = ? and c.istate = 0 where a.ibillid = b.id and a.cuserid = ? and a.operatortype <> 2 and (a.cbilldate <= ? or ichargetype = ?) and b.itype = 0 and a.ifunsid = ? and c.istate = 0",userid,userid,currentDate,@(SSJChargeIdTypeLoan),item.fundingID] - [db doubleForQuery:@"select sum(a.imoney) from bk_user_charge as a, bk_bill_type as b left join bk_share_books_member c on c.cbooksid = a.cbooksid and c.cmemberid = ? where a.ibillid = b.id and a.cuserid = ? and a.operatortype <> 2 and (a.cbilldate <= ? or ichargetype = ?) and b.itype = 1 and a.ifunsid = ? and c.istate = 01",userid,userid,currentDate,@(SSJChargeIdTypeLoan),item.fundingID];
