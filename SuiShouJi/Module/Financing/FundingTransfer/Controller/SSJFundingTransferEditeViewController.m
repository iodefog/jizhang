...
-(void)getTransferItemForCharge{
    ...
    [[SSJDatabaseQueue sharedInstance] asyncInDatabase:^(FMDatabase *db) {
        ...
        if ([weakSelf.chargeItem.billId integerValue] == 3) {
            ...
            tansferItem.transferOutImage = [db stringForQuery:@"select a.cicoin from bk_fund_info a, bk_user_charge b where a.cfundid = b.ifunsid and substr(b.cwritedate,1,19) = ? and a.cfundid <> ?",[weakSelf.chargeItem.editeDate substringWithRange:NSMakeRange(0, 19)],tansferItem.transferInId];
            ...
            tansferItem.transferInImage = [db stringForQuery:@"select a.cicoin from bk_fund_info a, bk_user_charge b where a.cfundid = b.ifunsid and substr(b.cwritedate,1,19) = ? and a.cfundid <> ?",[weakSelf.chargeItem.editeDate substringWithRange:NSMakeRange(0, 19)],tansferItem.transferOutId];
            tansferItem.transferMemo = weakSelf.chargeItem.chargeMemo;
