...
+ (void)queryDataWithFundTypeID:(NSString *)ID
    ...
    [[SSJDatabaseQueue sharedInstance] asyncInDatabase:^(FMDatabase *db) {
        FMResultSet *resultSet = [db executeQuery:@"select a.IMONEY, a.CBILLDATE , a.ICHARGEID , b.* , c.ITYPE from BK_USER_CHARGE as a, BK_BILL_TYPE as b , BK_BILL_TYPE as c where a.IBILLID = b.ID and a.IFID = ? and a.CBILLDATE like ?  and a.IBILLID = c.ID and a.operatortype != 2 order by a.CBILLDATE desc", ID, dateStr];
        if (!resultSet) {
