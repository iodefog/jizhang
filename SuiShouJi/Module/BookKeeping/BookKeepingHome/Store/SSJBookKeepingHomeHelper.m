                             ...
                             failure:(void (^)(NSError *error))failure
{
    ...
    [[SSJDatabaseQueue sharedInstance]asyncInDatabase:^(FMDatabase *db) {
        ...
        int count = 0;
        int chargeCount = 0;
        FMResultSet *chargeResult = [db executeQuery:@"SELECT A.CBILLDATE , A.IMONEY , A.ICHARGEID , A.IBILLID , A.CWRITEDATE  ,A.IFUNSID , A.CUSERID , A.CIMGURL ,  A.THUMBURL ,A.CMEMO , A.ICONFIGID , A.OPERATORTYPE AS CHARGEOPERATORTYPE , B.CNAME, B.CCOIN, B.CCOLOR, B.ITYPE FROM (SELECT CBILLDATE , IMONEY , ICHARGEID , IBILLID , CWRITEDATE  ,IFUNSID , CUSERID , CMEMO ,  CIMGURL ,  THUMBURL , ICONFIGID , OPERATORTYPE FROM (SELECT CBILLDATE , IMONEY , ICHARGEID , IBILLID , CWRITEDATE , IFUNSID , CUSERID , CMEMO ,  CIMGURL , THUMBURL , ICONFIGID , OPERATORTYPE , CBOOKSID FROM BK_USER_CHARGE WHERE OPERATORTYPE != 2 AND CBOOKSID = ?) WHERE IBILLID != '1' AND IBILLID != '2' AND IBILLID != '3' AND IBILLID != '4' AND CUSERID = ? UNION SELECT * FROM (SELECT CBILLDATE , SUMAMOUNT AS IMONEY , ICHARGEID , IBILLID , '3'||substr(cwritedate,2) AS CWRITEDATE , IFUNSID , CUSERID , '' AS CMEMO , '' AS CIMGURL , '' AS THUMBURL ,  0 AS OPERATORTYPE , CBOOKSID FROM BK_DAILYSUM_CHARGE WHERE CUSERID = ? AND CBOOKSID = ? ORDER BY CBILLDATE DESC)) AS A LEFT JOIN BK_BILL_TYPE AS B ON A.IBILLID = B.ID WHERE A.CBILLDATE <= ? ORDER BY A.CBILLDATE DESC , A.CWRITEDATE DESC",booksid,userid,userid,booksid,[[NSDate date]ssj_systemCurrentDateWithFormat:@"yyyy-MM-dd"]];
        if (!chargeResult) {
            if (failure) {
                SSJDispatch_main_async_safe(^{
                    failure([db lastError]);
