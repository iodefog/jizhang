                             ...
                             failure:(void (^)(NSError *error))failure
{
    ...
    [[SSJDatabaseQueue sharedInstance]asyncInDatabase:^(FMDatabase *db) {
        ...
        int count = 0;
        int chargeCount = 0;
        FMResultSet *chargeResult = [db executeQuery:@"select a.cbilldate , a.imoney , a.ichargeid , a.ibillid , a.cwritedate  ,a.ifunsid , a.cuserid , a.cimgurl ,  a.thumburl ,a.cmemo , a.iconfigid , a.operatortype as chargeoperatortype , b.cname, b.ccoin, b.ccolor, b.itype from (select cbilldate , imoney , ichargeid , ibillid , cwritedate  ,ifunsid , cuserid , cmemo ,  cimgurl ,  thumburl , iconfigid , operatortype from (select cbilldate , imoney , ichargeid , ibillid , cwritedate , ifunsid , cuserid , cmemo ,  cimgurl , thumburl , iconfigid , operatortype , cbooksid from bk_user_charge where operatortype != 2 and cbooksid = ?) where ibillid like '2%' or ibillid like '1%' or length(ibillid) > 4 and cuserid = ? union select * from (select cbilldate , sumamount as imoney , '' as ichargeid , '-1' as ibillid , '3'||substr(cwritedate,2) as cwritedate , '' as ifunsid , cuserid , '' as cmemo , '' as cimgurl , '' as thumburl ,  0 as operatortype , cbooksid from bk_dailysum_charge where cuserid = ? and cbooksid = ? order by cbilldate desc)) as a left join bk_bill_type as b on a.ibillid = b.id where a.cbilldate <= ? order by a.cbilldate desc , a.cwritedate desc",booksid,userid,userid,booksid,[[NSDate date]ssj_systemCurrentDateWithFormat:@"yyyy-MM-dd"]];
        if (!chargeResult) {
            if (failure) {
                SSJDispatch_main_async_safe(^{
                    failure([db lastError]);
