...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [SSJBudgetDatabaseHelper queryForCurrentBudgetListWithSuccess:^(NSArray<SSJBudgetModel *> * _Nonnull result) {
        if (result.count != 0) {
            ...
            self.remindView.model = [result objectAtIndex:0];
    ...
    [[SSJDatabaseQueue sharedInstance]asyncInTransaction:^(FMDatabase *db , BOOL *rollback){
        ...
        while ([rs next]) {
            ...
            if (item.chargeImage.length > 0) {
                item.chargeThumbImage = [NSString stringWithFormat:@"%@-thumb.jpg",[item.chargeImage substringToIndex:item.chargeImage.length - 4]];
            }
