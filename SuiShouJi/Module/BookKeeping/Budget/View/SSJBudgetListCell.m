...
- (void)layoutSubviews {
    ...
    self.typeLab.leftTop = CGPointMake(10, 15);
}
...
- (void)setCellItem:(SSJBaseItem *)cellItem {
    ...
    self.beginDateLab.text = item.beginDate;
    ...
    if (item.payment <= item.budget) {
        ...
        [self.waveView setTopTitle:@"支出"];
        [self.waveView setBottomTitle:[NSString stringWithFormat:@"%f", item.payment]];
    } else {
        [self.waveView setScale:0.7];
        [self.waveView setTopTitle:@"超出"];
