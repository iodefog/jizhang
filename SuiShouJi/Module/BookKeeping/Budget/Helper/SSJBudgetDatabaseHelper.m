...
//    }];
    ...
    [[SSJDatabaseQueue sharedInstance] asyncInDatabase:^(FMDatabase *db) {
        BOOL isConficted = [db boolForQuery:@"select count(*) from bk_user_budget where cuserid = ? and operatortype <> 2 and ibid <> ? and itype = ? and csdate <= datetime('now', 'localtime') and cedate >= datetime('now', 'localtime') and cbooksid = ?", SSJUSERID(), model.ID, @(model.type), model.beginDate, model.booksId];
        if (success) {
            SSJDispatch_main_async_safe(^{
                success(isConficted);
            });
