...
+ (NSError *)createUserBillTypeTableWithDatabase:(FMDatabase *)db {
    if (![db executeUpdate:@"CREATE TABLE IF NOT EXISTS BK_USER_BILL_TYPE (CBILLID TEXT, CUSERID TEXT, CBOOKSID TEXT, ITYPE INTEGER, CNAME TEXT, CCOLOR TEXT, CICOIN TEXT, IORDER INTEGER, CWRITEDATE TEXT, OPERATORTYPE INTEGER, IVERSION INTEGER, ISTATE INTEGER DEFAULT 1, PRIMARY KEY(CBILLID, CUSERID, CBOOKSID))"]) {
        return db.lastError;
    }
    return nil;
}
