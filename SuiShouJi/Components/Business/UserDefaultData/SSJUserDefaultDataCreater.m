...
+ (NSError *)createDefaultFundAccountsForUserId:(NSString *)userId inDatabase:(FMDatabase *)db {
    ...
    [db executeUpdate:@"INSERT INTO BK_FUND_INFO (CFUNDID, CACCTNAME, CPARENT, CCOLOR, CWRITEDATE, OPERATORTYPE, IVERSION, CUSERID, CICOIN, IORDER) VALUES (?, '借出款', '10', '', ?, 0, ?, ?, '', 5)", [NSString stringWithFormat:@"%@-5",userId], writeDate, @(SSJSyncVersion()), userId];
    [db executeUpdate:@"INSERT INTO BK_FUND_INFO (CFUNDID, CACCTNAME, CPARENT, CCOLOR, CWRITEDATE, OPERATORTYPE, IVERSION, CUSERID, CICOIN, IORDER) VALUES (?, '欠款', '11', '', ?, 0, ?, ?, '', 6)", [NSString stringWithFormat:@"%@-6",userId], writeDate, @(SSJSyncVersion()), userId];
...
+ (NSError *)createDefaultBooksTypeForUserId:(NSString *)userId inDatabase:(FMDatabase *)db {
    ...
    [db executeUpdate:@"INSERT INTO BK_BOOKS_TYPE (CBOOKSID, CBOOKSNAME, CBOOKSCOLOR, CWRITEDATE, OPERATORTYPE, IVERSION, CUSERID , IORDER ,CICOIN) VALUES (?, ?, ?, ?, 0, ?, ? , ? , ?)", [NSString stringWithFormat:@"%@-3",userId], @"装修账本", @"#5ca0d9", writeDate, @(SSJSyncVersion()), userId,@(4),@"book_zhuangxiu"];
